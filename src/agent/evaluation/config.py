...
class EvalConfig(BaseModel):
    ...
    # Agent
    top_k: int = 20
    rerank_top_n: int = 20  # matches the best B config; 5 under-retrieves
    max_iterations: int = 8  # recursion_limit = 2*this+1; the loop is now multi-hop
    ...
    def from_cli(cls, argv: list[str] | None = None) -> "EvalConfig":
        ...
        p.add_argument("--no-force-first-tool", dest="force_first_tool",
                       action="store_false", default=d.force_first_tool)
